...
**⚠️ External JS Files:** Do not edit the `.js` files in `static/hexmap/` that are in `.gitignore` (e.g., `game.js`, `auth.js`, `cloud-storage.js`). These are generated/maintained externally in the sibling `holmgard-lore-game` project. Our custom patches (e.g., `game-ui-bindings.js`) are the only editor-maintained JS files in that directory.
